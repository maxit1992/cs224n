...
\ifans{
The masks are used to assign $-inf$ to $e_{t,i}$ where the mask is 1 (i.e. padding words indexes). By doing so, when computing the softmax, the exponential will return a very low value (zero in the limit) at mask 1 positions, so those positions won't be considered when normalizing $e_{t}$ to probabilities. As a result, no attention will be assigned to padding words.
}
...
\ifans{
\begin{itemize}
    \item An advantage of dot product attention is that it is faster to calculate because it is a simple dot product without trainable parameters. As a disadvantage, the multiplicative attention gives us the possibility to make a more effective use of the space by adding a trainable matrix in the product.
    \item The additive attention gives us more representation capabilities by using different matrix for the input vectors and also it is capable of modeling non linear relationships between the input vectors and the attention score. As a disadvantage, the computation is more complex and requires one additional step (the execution of the non linear activation function).
\end{itemize}
}
