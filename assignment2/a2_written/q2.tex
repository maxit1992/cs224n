    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                \ifans{
                Being $\beta_1=0.9$, most of the value of $m_t$ is derived from the previous value (history). Hence, the novelty given by the gradient is reduced and the variance in the direction of the movement is reduced. This may be helpful to escape from local minimums, since it gives us a momentum in the movement that explores the nearby space of the local minimums in the loss surface. 
                }
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                \ifans{
                This trick creates an effective $\alpha$ that is normalized for each update dimmension, given the size of the gradient for earch dimmension. Because of this, the model parameters with smaller gradients will receive larger updates. This enhance the learning because we generate homogeneous learn in all parameters.
                }
        ...
        \ifans{
            $\gamma = \frac{1}{1-p_{drop}}$ If we nullify $p_{drop}$ of the values at the output, the next layer will sea a reduction of around $(1-p_{drop})\%$ of the acumulated value. To normalize this situation we have to scale the output to keep the range of values seeen by the next layer.
        }
          ...
          \ifans{
          Applying dropout during \textbf{training} helps reducing the overfitting, because it works similar to boosting algorithms: zeroing outputs is the same as reducing neuron connections, thus reducing the effective network complexity and reducing the probability of overfitting. The knowledge is then distributed along the network and the deeper layers of the networks learns how to "merge" the distributed knowledge (as the decision step that merges the output of simple models does in boosting algorithms).
          During evaluation, we need all the network capacity and learnt knowledge to make predictions, otherwise we would directly use simpler networks. Because of this, we don't apply the dropout network during \textbf{evaluation}.
          }
